...
class Mujoco(Interface):
    ...
    def connect(self, joint_names=None, camera_id=-1, sim = None, **kwargs):
        """
        ...
        # [------------------ POSIBLE ERROR DE LIBRERIA------------------]
        # LA UNION LIBRE TIENE 6 DOF, PERO ACA SE PONE EL RANGE DE 1 A 6,
        # LUEGO SE MUEVE EL INDICE 6, PERO EL LEN(RANGE(1, 6)) = 5!!!, SE VA
        # A CAMBIAR RANGE(1, 7), SUPONGO QUE ESO LO ARREGLAR.
        ...
        for ii, joint_type in enumerate(model.jnt_type):
            ...
            if joint_type == 0:  # free joint
                self.joint_dyn_addrs += [jj + index for jj in range(1, 6 + 1)]
                index += 6  # derivative has 6 dimensions
            elif joint_type == 1:  # ball joint
                self.joint_dyn_addrs += [jj + index for jj in range(1, 3 + 1)]
