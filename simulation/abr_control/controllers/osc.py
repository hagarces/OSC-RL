...
class OSC(Controller):
    ...
    def generate(
        ...
        if self.ki != 0:
            ...
            if self.int_decay and self.int_thresh and self.int_limit:
                # que no siga agregando error si es que ya llego
                #u_task = np.where(abs(u_task) > self.int_thresh, u_task, 0)
                # que el error decaiga cuando esta cerca
                self.integrated_error = self.integrated_error * self.int_decay + u_task
                # Evitar que crezca mucho (antiwind up)
                self.integrated_error = np.where(self.integrated_error > self.int_limit,
                                                 self.int_limit, self.integrated_error)
                self.integrated_error = np.where(self.integrated_error < -self.int_limit,
                                                 -self.int_limit, self.integrated_error)
